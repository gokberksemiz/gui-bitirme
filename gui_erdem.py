...
# Function for choosing the background color of the Canvas    
    ...
    def paint_processed_data(self):
        ...
        self.processed_data = eng.guismoth2(x,y,speed,M,nargout=4)
        ...
        self.background.delete('all')    
        prev_x, prev_y, prev_speed = self.processed_data[0][0], self.processed_data[1][0], self.processed_data[2][0]
        #prev_color = self.get_color(prev_speed)
        last_data_point = len(self.processed_data[0][0])
        ...
        print(prev_x,prev_y,prev_speed)
        self.coords_list = [[prev_x[i],prev_y[i],prev_speed[i]] for i in range(len(self.processed_data[0][0]))]
    ...
    def send_data(self):
        ...
        sock.connect(('192.168.137.108', 8080))
